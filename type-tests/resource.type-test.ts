...
void paginatedUsers.links;

export type ResourceTypeTests = {
  make: MakeReturnsConcreteResource;
  collection: CollectionReturnsConcreteResources;
  paginatedData: PaginatedDataKeepsConcreteResource;
  defaultPage: DefaultPaginationPageIsNumber;
  defaultPages: DefaultPaginationPagesIsNumber;
  nestedMake: NestedMakeReturnsNamedResource;
  nestedName: NestedResourceKeepsItsName;
  nestedCollection: NestedCollectionReturnsNamedResources;
};
...
const ProjectResource = Resource.configure({
  pagination: {
    ...
    links(state, context) {
      ...
      const query: Readonly<Record<string, unknown>> | undefined =
        context.query;
      const filter: Readonly<Record<string, unknown>> | undefined =
        context.filter;
      const sort: Readonly<Record<string, unknown>> | undefined = context.sort;
      ...
      return {
        next: state.hasNextPage ? `${path ?? '/orders'}?page=2` : null,
      };
    },
  },
});
